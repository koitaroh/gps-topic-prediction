...
def load_grid_mode_dataset(X_GRID_FILE, Y_GRID_FILE, X_MODE_FILE, Y_MODE_FILE, le_grid, lb_mode, EXPERIMENT_PARAMETERS):
    ...
    return X_train_all, y_train_all, X_test, y_test, X_mode_train_all, X_mode_test, y_mode_train_all, y_mode_test
...
def training_lstm_grid_mode(X_train, y_train, X_test, y_test, X_mode_train, X_mode_test, y_mode_train, y_mode_test, EXPERIMENT_PARAMETERS, max_s_index, MODEL_FILE, MODEL_WEIGHT_FILE_LSTM_GRID, FIGURE_DIR):
    '''
    ...
    model = create_model_lstm_grid_mode(input_shape, input_mode_shape, max_s_index)
    es_cb = EarlyStopping(monitor='val_loss', patience=10, verbose=1, mode='auto')
    model.summary()
    history = model.fit([X_train, X_mode_train], y_train, batch_size=50, epochs=50, validation_data=([X_test, X_mode_test], y_test_one_step), callbacks=[es_cb])
...
def create_model_lstm_grid_mode(input_shape, input_mode_shape, max_s_index):
    hidden_neurons = 128
...
def prediction_multiple_steps_lstm_grid_mode(model, X, y, X_mode, y_mode, le_grid, EXPERIMENT_PARAMETERS):
...
if __name__ == '__main__':
    ...
    X_train, y_train, X_test, y_test, X_mode_train, X_mode_test, y_mode_train, y_mode_test, = load_grid_mode_dataset(X_GRID_FILE, Y_GRID_FILE, X_MODE_FILE, Y_MODE_FILE, le_grid, lb_mode, EXPERIMENT_PARAMETERS)
    max_s_index = len(le_grid.classes_)
    # np.save(LE_GRID_CLASSES_FILE, le_grid.classes_)
    # print(X_train)

    # Train model
    lstm_model = training_lstm_grid_mode(X_train, y_train, X_test, y_test, X_mode_train, X_mode_test, y_mode_train, y_mode_test, EXPERIMENT_PARAMETERS, max_s_index, MODEL_FILE_LSTM_GRID, MODEL_WEIGHT_FILE_LSTM_GRID, FIGURE_DIR)

    # Load model
    lstm_model = load_model(str(MODEL_FILE_LSTM_GRID))

    y_predicted_sequence, error_meter_series = prediction_multiple_steps_lstm_grid_mode(lstm_model, X_test, y_test, X_mode_test, y_mode_test, le_grid, EXPERIMENT_PARAMETERS)
