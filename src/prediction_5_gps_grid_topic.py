...
def load_grid_topic_dataset(X_GRID_FILE, Y_GRID_FILE, X_TOPIC_FILE, Y_TOPIC_FILE, le_grid, EXPERIMENT_PARAMETERS):
    ...
    return X_train_all, y_train_all, X_test, y_test, X_topic_train_all, X_topic_test, y_topic_train_all, y_topic_test
...
def training_lstm_grid_topic(X_train, y_train, X_test, y_test, X_topic_train, X_topic_test, y_topic_train, y_topic_test, EXPERIMENT_PARAMETERS, max_s_index, MODEL_FILE, MODEL_WEIGHT_FILE_LSTM_GRID, FIGURE_DIR):
    '''
    ...
    model = create_model_lstm_grid_topic(input_shape, input_topic_shape, max_s_index)
    es_cb = EarlyStopping(monitor='val_loss', patience=10, verbose=1, mode='auto')
    model.summary()
    history = model.fit([X_train, X_topic_train], y_train, batch_size=50, epochs=50, validation_data=([X_test, X_topic_test], y_test_one_step), callbacks=[es_cb])
    ...
    loss, accuracy, sparse_top_k_accuracy = model.evaluate([X_test, X_topic_test], y_test_one_step, batch_size=300)
...
def create_model_lstm_grid_topic(input_shape, input_topic_shape, max_s_index):
    hidden_neurons = 128
    ...
    lstm_input = Concatenate()([x_input_embedding, x_topic_input])
    ...
    model = Model(inputs=[x_input, x_topic_input], outputs=[main_output])
    model.compile(optimizer='rmsprop', loss='sparse_categorical_crossentropy', metrics=['accuracy', 'sparse_top_k_categorical_accuracy'])
    return model


def prediction_multiple_steps_lstm_grid_topic(model, X, y, X_topic, y_topic, le_grid, EXPERIMENT_PARAMETERS):
    ...
    for i in tqdm(range(PREDICTION_OUTPUT_LENGTH)):
        if i == 0:
            y_predicted = model.predict([X_middle_step, X_topic_middle_step])
            y_true = y[:, i, :]
...
#     # print(y_join)
#     # print(y_join.shape)
#     error_meter_series = np.apply_along_axis(calculate_rmse_in_meters, axis=1, arr=y_join)
...
if __name__ == '__main__':
    ...
    X_train, y_train, X_test, y_test, X_topic_train, X_topic_test, y_topic_train, y_topic_test = load_grid_topic_dataset(X_GRID_FILE, Y_GRID_FILE, X_TOPIC_FILE, Y_TOPIC_FILE, le_grid, EXPERIMENT_PARAMETERS)
    max_s_index = len(le_grid.classes_)
    # np.save(LE_GRID_CLASSES_FILE, le_grid.classes_)
    # print(X_train)

    # Train model
    lstm_model = training_lstm_grid_topic(X_train, y_train, X_test, y_test, X_topic_train, X_topic_test, y_topic_train, y_topic_test, EXPERIMENT_PARAMETERS, max_s_index, MODEL_FILE_LSTM_GRID, MODEL_WEIGHT_FILE_LSTM_GRID, FIGURE_DIR)

    # Load model
    lstm_model = load_model(str(MODEL_FILE_LSTM_GRID))

    y_predicted_sequence, error_meter_series = prediction_multiple_steps_lstm_grid_topic(lstm_model, X_test, y_test, X_topic_test, y_topic_test, le_grid, EXPERIMENT_PARAMETERS)
