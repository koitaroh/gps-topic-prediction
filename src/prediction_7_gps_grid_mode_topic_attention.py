...
def load_grid_mode_topic_dataset(X_GRID_FILE, Y_GRID_FILE, X_MODE_FILE, Y_MODE_FILE, X_TOPIC_FILE, Y_TOPIC_FILE, le_grid, lb_mode, EXPERIMENT_PARAMETERS):
    ...
    X_mode_all = np.load(X_MODE_FILE, allow_pickle=True)
    y_mode_all = np.load(Y_MODE_FILE, allow_pickle=True)
    ...
    X_topic_all = np.load(X_TOPIC_FILE, allow_pickle=True)
    y_topic_all = np.load(Y_TOPIC_FILE, allow_pickle=True)
...
def training_lstm_grid_mode_topic(X_train, y_train, X_test, y_test, X_mode_train, X_mode_test, y_mode_train, y_mode_test, X_topic_train, X_topic_test, y_topic_train, y_topic_test, EXPERIMENT_PARAMETERS, max_s_index, MODEL_FILE, MODEL_WEIGHT_FILE_LSTM_GRID, FIGURE_DIR):
    '''
    ...
    model = create_model_lstm_grid_mode_topic(input_shape, input_mode_shape, input_topic_shape, max_s_index)
    es_cb = EarlyStopping(monitor='val_loss', patience=10, verbose=1, mode='auto')
    model.summary()
    history = model.fit([X_train, X_mode_train, X_topic_train], y_train, batch_size=50, epochs=100, validation_data=([X_test, X_mode_test, X_topic_test], y_test_one_step), callbacks=[es_cb])
...
def create_model_lstm_grid_mode_topic(input_shape, input_mode_shape, input_topic_shape, max_s_index):
    hidden_neurons = 128
...
def prediction_multiple_steps_lstm_grid_mode_topic(model, X, y, X_mode, y_mode, X_topic, y_topic, le_grid, EXPERIMENT_PARAMETERS):
...
if __name__ == '__main__':
    ...
    # Train model
    lstm_model = training_lstm_grid_mode_topic(X_train, y_train, X_test, y_test, X_mode_train, X_mode_test, y_mode_train, y_mode_test, X_topic_train, X_topic_test, y_topic_train, y_topic_test, EXPERIMENT_PARAMETERS, max_s_index, MODEL_FILE_LSTM_GRID, MODEL_WEIGHT_FILE_LSTM_GRID, FIGURE_DIR)

    # Load model
    lstm_model = load_model(str(MODEL_FILE_LSTM_GRID))

    y_predicted_sequence, error_meter_series = prediction_multiple_steps_lstm_grid_mode_topic(lstm_model, X_test, y_test, X_mode_test, y_mode_test, X_topic_test, y_topic_test, le_grid, EXPERIMENT_PARAMETERS)
