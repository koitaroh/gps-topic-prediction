...
def establish_db_connection_postgresql_geotweet_remote():
    ...
    # engine = sqlalchemy.create_engine(ENGINE_CONF,  echo=True)
    engine = sqlalchemy.create_engine(ENGINE_CONF)
    conn = engine.connect()
    metadata = sqlalchemy.MetaData(ENGINE_CONF)
    return engine, conn, metadata


def establish_db_connection_postgresql_geotweet(EXPERIMENT_ENVIRONMENT):
    if EXPERIMENT_ENVIRONMENT == "remote":
        engine, conn, metadata = establish_db_connection_postgresql_geotweet_remote()
    elif EXPERIMENT_ENVIRONMENT == "local" or "win":
        engine, conn, metadata = establish_db_connection_postgresql_geotweet_ssh()
    return engine, conn, metadata
...
def establish_db_connection_mysql_twitter(EXPERIMENT_ENVIRONMENT):
    if EXPERIMENT_ENVIRONMENT == "remote":
        engine, conn, metadata = establish_db_connection_mysql_twitter_remote()
    elif EXPERIMENT_ENVIRONMENT == "local" or "win":
