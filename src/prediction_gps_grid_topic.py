...
def load_grid_topic_dataset(X_GRID_FILE, Y_GRID_FILE, X_TOPIC_FILE, Y_TOPIC_FILE, EXPERIMENT_PARAMETERS):
...
def training_lstm_grid(X_train, y_train, X_test, y_test, X_topic_train, X_topic_test, y_topic_train, y_topic_test, EXPERIMENT_PARAMETERS, max_s_index, MODEL_FILE, MODEL_WEIGHT_FILE_LSTM_GRID, FIGURE_DIR):
    '''
    ...
    print('input mode shape:', input_topic_shape)
    ...
    x_input_embedding = Embedding(input_dim=max_s_index, output_dim=256, input_length=X_train.shape[1])(x_input_flatten)

    x_mode_input = Input(shape=input_topic_shape)
    ...
    lstm_input = Concatenate()([x_input_embedding, x_mode_input])
    # lstm_1 = LSTM(hidden_neurons, return_sequences=True, dropout=0.2, recurrent_dropout=0.2, name='lstm-1', activation='softsign')(lstm_input)
    # lstm_2 = LSTM(hidden_neurons, return_sequences=False, dropout=0.2, recurrent_dropout=0.2, name='lstm-2', activation='softsign')(lstm_1)
    ...
    # lstm_1 = LSTM(hidden_neurons, return_sequences=True, name='lstm-1')(lstm_input)
    # lstm_2 = LSTM(hidden_neurons, return_sequences=False, name='lstm-2')(lstm_1)
    ...
    model.summary()
    history = model.fit([X_train, X_topic_train], y_train, batch_size=batch_size, epochs=50, validation_data=([X_test, X_topic_test], y_test_one_step), callbacks=[es_cb])
...
if __name__ == '__main__':
    ...
    X_train, y_train, X_test, y_test, X_topic_train, X_topic_test, y_topic_train, y_topic_test = load_grid_topic_dataset(X_GRID_FILE, Y_GRID_FILE, X_TOPIC_FILE, Y_TOPIC_FILE, EXPERIMENT_PARAMETERS)

    # Train model
    lstm_model = training_lstm_grid(X_train, y_train, X_test, y_test, X_topic_train, X_topic_test, y_topic_train, y_topic_test, EXPERIMENT_PARAMETERS, max_s_index, MODEL_FILE_LSTM_GRID, MODEL_WEIGHT_FILE_LSTM_GRID, FIGURE_DIR)

    # Load model
    lstm_model = load_model(MODEL_FILE_LSTM_GRID)

    y_predicted_sequence = prediction_multiple_steps(lstm_model, X_test, y_test, X_topic_test, y_topic_test, spatial_index, EXPERIMENT_PARAMETERS)
